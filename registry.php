...
<head>
  ...
  <title>Registry Mail | Uniben</title>
</head>
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
  ...
  <main class="cd-main-content">
    <nav class="cd-side-nav js-cd-side-nav" style="font-size:20px">
      <ul class="cd-side__list js-cd-side__list">
        ...
        <ul class="cd-side__sub-item">
            <li class="cd-side__sub-item"><a href="registry.php" style="color:#BEC9B7; font-size:14px">Registry<br> Mails: <?php $registry->mails('registry') ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=bursary" name="category" style="color:#BEC9B7; font-size:14px">Bursary<br> Mails: <?php $bursary->mails('bursary'); ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=ICT/CRPU" name="category" style="color:#BEC9B7; font-size:14px">ICT/CRPU<br> Mails: <?php $ICT->mails('ICT/CRPU') ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=servicom" name="category" style="color:#BEC9B7; font-size:14px">Servicom<br> Mails: <?php $servicom->mails('servicom') ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=transcript" name="category" style="color:#BEC9B7; font-size:14px">Transcript<br> Mails: <?php $transcript->mails('transcript') ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=student affairs" name="category" style="color:#BEC9B7; font-size:14px">Student Affairs<br> Mails: <?php $student_affairs->mails('student affairs') ?></a></li>
            <li class="cd-side__sub-item"><a href="regview.php?category=others" name="category" style="color:#BEC9B7; font-size:14px">Others<br> Mails: <?php $others->mails('others') ?></a></li>
          </ul>
	...
	$result_count = mysqli_query($link,"SELECT COUNT(*) As total_records FROM `contact_us` WHERE categories='registry'");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    ...
    //number of unread messages
    $result_count_unread = mysqli_query($link,"SELECT COUNT(*) As total_unread FROM `contact_us` WHERE categories='registry'AND action='Unread'");
    $total_unread = mysqli_fetch_array($result_count_unread);
    $total_unread = $total_unread['total_unread'];
    echo "<div></div><h3>Total Mail: $total_records | Unread: $total_unread</h3></div>";

    $result = mysqli_query($link,"SELECT * FROM `contact_us` WHERE categories='registry' ORDER BY id DESC LIMIT $offset, $total_records_per_page");
    ...
    <div style='display:flex'><input type='checkbox' name='mail[]' value=$id> <div style=padding-left:10px;>".$action."</div></div>
              ...
              <div style='float:right; margin-right:10px;'> <a href='regmsg.php?button=" . $row["id"]."&page_no=" .$page_no . "'><button type='button' value='button2' class='btn btn-primary'><span class='glyphicon glyphicon-eye-open'></span>View message</button></a> </div>   
            ...
            echo "<div style='float:right; display:flex; margin-right:10px;'><select type='select' name='category1' value='category1' >                    
              ...
              <input type='submit' name='apply' value='Apply' id='submit' class='btn btn-primary'><br>
              <span>$resultMessages</span>
              </select></div>
               </form>";
	mysqli_close($link);
    ?>
